...
public class WallRunTutorial : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (isWallRunning)
        {
            Camera.main.fieldOfView = (76);
        }
        if (!isWallRunning)
        {
            Camera.main.fieldOfView = 72;
        }
        if ((Input.GetKey("w") || Input.GetKey("a") || Input.GetKey("s") || Input.GetKey("d"))&& grounded)
        {
            walk.volume = 0.1f;
        }
        else
        ...
        //if (rb.velocity.x > 10||rb.velocity.y > 10|| rb.velocity.z > 10)
        //{
        //    playerCam.GetComponent<CameraShaker>().ShakeOnce(5f,5f,5f,5f);
        //}
    }
